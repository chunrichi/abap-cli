...
import { SUPPORTED_WHERE_USED_TYPES, DEFAULT_WHERE_USED_LIMIT, MAX_WHERE_USED_LIMIT } from './where-used-ops.js';
...
function pullSchema(): CommandSchema {
  return {
    ...
    options: [
      ...
      { name: '--textpool', type: 'boolean', description: '014: also pull textpool files (.texts/.selections/.headings.<lang>.properties).' },
      { name: '--remote', type: 'string', valuePlaceholder: '<remoteid>', description: '015: pull the object\'s active version source from a remote system (Version Management).' },
      { name: '--tr', type: 'string', valuePlaceholder: '<request>', description: 'T4.2: pull all objects bound to a transport request (mutually exclusive with object name and --package).' },
      schemaOption(),
    ],
