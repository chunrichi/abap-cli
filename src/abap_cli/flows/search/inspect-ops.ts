...
export interface ActivationPart {
   ...
   * known SAP/ADT semantic quirk (currently: the `source/main` INCLUDE of OO
   * classes, whose active version is system-regenerated and never byte-equal
   * to the user-written latest). Agents should not act on `active: false`
   * for parts carrying a note — the class's real activation state lives in
 ...
 *
 * For OO classes/interfaces (CLAS/OC, INTF/OI) the `source/main` INCLUDE is
 * a system-managed artefact whose active version is regenerated by SAP with
 * case-lowering, `create private .`, and synthetic section headers — so its
 * bytes never match the user-written latest. Such parts get a `note` to
 * disambiguate; the `ok` flag is driven by the implementation parts only.
 */
async function checkActivation(
  ...
  // OO objects (classes / interfaces) have a system-managed `source/main`
  // INCLUDE that SAP regenerates server-side. Detect that case so we can
  // annotate the `main` part with a note explaining the inevitable mismatch.
  const OO_TYPES = new Set(['CLAS/OC', 'INTF/OI']);
  const isOOClass = OO_TYPES.has(objectType);
  const OO_MAIN_NOTE =
    'SAP-managed INCLUDE: active version is regenerated server-side ' +
    '(lowercase / create-private / synthetic section headers) and will ' +
    'never byte-match the user-written latest. The class activation ' +
    'state is determined by the implementation parts above, not by ' +
    'this flag.';
  for (const inc of includes) {
    ...
    const part: ActivationPart = { includeType, sourceUri, active };
    if (isOOClass && includeType === 'main') {
      part.note = OO_MAIN_NOTE;
    }
    parts.push(part);
  }
  // OO-class `includeType:main` reports active=false for the system-managed
  // INCLUDE program regardless of whether the class is fully activated.
  // Implementation parts (`implementations` / `definitions` / `testclasses` /
  // `macros`) are the source of truth — `main` carries a per-part `note`
  // (see above) and is excluded from the ok flag.
