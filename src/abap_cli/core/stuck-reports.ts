...
let reportsDir = path.join(userConfigDir(), 'reports');
...
function reportsAncestorIsWritable(): boolean {
  let current = path.resolve(reportsDir);
  while (true) {
    try {
      const stat = fs.statSync(current);
      if (!stat.isDirectory() || (stat.mode & 0o222) === 0) {
        return false;
      }
      return true;
    } catch (error: unknown) {
...
export function writeStuckReport(input: StuckReportInput): StuckReportResult {
  ...
  try {
    if (!reportsAncestorIsWritable()) throw new CliError('FILE_PARSE_ERROR', `Could not write stuck report to ${reportsDir}.`);
...
let counterPath = path.join(userConfigDir(), '.stuck-count.json');
