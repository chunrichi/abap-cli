...
import { AdtClientWrapper } from './adt-client.js';
...
export async function recordCapability(systemName: string, cap: TextpoolCapability & { systemVersion?: string }): Promise<void> {
  ...
  if (!current) {
    throw new CliError('CONFIG_ERROR', `System profile '${systemName}' not found; cannot record textpool capability`, {
    nextSteps: [`Re-run abap init to create profile '${systemName}'.`],
  });
  }
