...
- Pull is now organized behind a per-type `PullStrategy` (`src/abap_cli/formats/pull-strategy.ts`): CLAS/PROG/INTF share the objectStructure + source-parts layout, and the strategy is the extension point for future types (e.g. FUGR). Unsupported pull types fail with `TYPE_NOT_SUPPORTED`.
