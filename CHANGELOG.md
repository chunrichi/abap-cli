...
### Changed
- **push 对 `$TMP` 对象的 `--tr` 语义**（`specs/035-push-tr-semantics/`）：`$TMP` 对象 transport-free,显式传非空 `--tr` 不再静默吞掉——改为 `meta.warnings` 的 `PUSH_TR_IGNORED_TMP` 告警 + 忽略(exit 不变),避免 agent 误以为 `--tr` 生效。判定提前到 transportInfo 之前,顺带省掉 `$TMP` 对象的请求归属查询。
- **push 对不存在对象的引导**（`specs/035-push-tr-semantics/`）：ADT 源对象 `OBJECT_NOT_FOUND` 的 `nextSteps` 增补 `abap create <TYPE> <name>` 引导;DDIC/TRAN push 在 POST 前 ICF GET 探测,不存在 → `OBJECT_NOT_FOUND` + 引导 `abap create`(HTTP 例外,见 Removed)。
...
### Removed (breaking)
- **push 对 DDIC/TRAN 的隐式创建**（`specs/035-push-tr-semantics/`）：`abap push <file>.json` 一个 SAP 里不存在的 DOMA/DTEL/TABL/STRU/TRAN 对象,以前经 ICF `POST /ddic|/tran`(upsert)静默创建;现在 push 前 ICF GET 探测,对象不存在 → `OBJECT_NOT_FOUND`(exit 8)+ 引导 `abap create <TYPE> <name> --file <path> --package ... --tr ...`。迁移:创建新 DDIC/TRAN 对象改用 `abap create`(创建与归属由 `create --package/--tr` 负责,改归属用 `abap transport assign`)。**例外**:HTTP(SICF 节点)push 保留 push 即创建——`create HTTP` 无 `--file` 只落本地骨架,建 SICF 节点依赖 `abap push`。
