...
### Added
- **030 — ADT runtime 分层 + Steampunk `extension deploy` 分流**：新 `src/abap_cli/adc/runtime-probe.ts` 探测三档 ADT runtime（`netweaver740` / `netweaver750` / `steampunk` / `unknown`），依据 `/sap/bc/adt/repository/informationsystem` Atom XML 的 `sap-component`（首选）+ `/sap/bc/adt/discovery` workspace 的 `/sap/bc/adt/icf/*` collection（备用）。`IcfDeploymentInfo` 加 `runtime` + `icfSetupBlocked` 字段；`extension status` JSON 输出含 `runtime` / `icfSetupBlocked`；`extension deploy` 在 Steampunk 下走 `deployKind: "source-only"`，sources 照常 deploy，ICF 节点注册改为 `meta.warnings[].code: "STEAMPUNK_ICF_MANUAL"` + human 输出 Cloud Foundry destination 步骤（不抛错）。on-prem 用户行为**完全无感**（仍走 `cl_icf_tree`）。设计 + 决策见 `specs/030-btp-ext-deploy-strategy/spec.md`。
