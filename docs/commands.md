# Commands Reference (auto-generated — v0.2.6)
...
```jsonc
// Success (stdout)
{ "status": "success", "meta": { "command": "abap pull", "version": "0.2.6", "timestamp": "...", "durationMs": 42, "warnings": [] }, "data": { ... } }
...
Deploy bundled ICF ABAP service to SAP.

```bash
abap deploy [options] | abap deploy status [options]
```
...
abap deploy --package ZPKG --tr DEVK900001 --yes
...
- Unified envelope contract: `src/abap_cli/output/cli-output.schema.json` (envelope shape) + `src/abap_cli/output/cli-output-codes.schema.json` (exit-code and error-code maps)
