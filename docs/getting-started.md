...
- **Textpool (text elements)** — **read** program text symbols, selection texts and list headings via `abap pull <name> --textpool`. **Write** is system-dependent and must not be assumed: it needs the ADT text-elements write endpoint, and on releases that only expose the non-interactive textpool API it is unavailable — verified on A4H, where `abap push <name>.<type>.texts|selections|headings.<lang>.properties` returns `TEXTPOOL_WRITE_UNSUPPORTED` (see [push](../wiki/commands/push.md) and the textpool variant in the `abap-cli-edit` skill). Mixed mode: the ADT text-elements API is used when the system supports it; ECC/older systems route through the ICF `/textpool/*` endpoint. Capability is probed once when the connection profile is created (`abap profile add/set`, `abap init`) and cached in the profile's `adtTextpool` — no runtime fallback (see [Commands](commands.md) for details).
