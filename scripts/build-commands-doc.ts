...
const { commandSchemas, searchCommandSchema, whereUsedCommandSchema } = await import(flows('command-schemas.js'));
const { createSchema } = await import(flows('create-schema.js'));
...
sections.push('');
sections.push('> **Source of truth**: this file is generated by `scripts/build-commands-doc.ts`. Do not edit by hand — change the command schema (`src/abap_cli/commands/*.ts`, `src/abap_cli/flows/command-schemas.ts`) and run the generator.');
sections.push('');
...
sections.push('- [`src/abap_cli/output/help-text.ts`](src/abap_cli/output/help-text.ts) — `commonErrorsAfter()` (the inline help block on every command)');
sections.push('');
