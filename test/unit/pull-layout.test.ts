...
describe('abap pull (abap-file-format layout)', () => {
  it('writes one directory per object with .json metadata + .abap source (main part)', async () => {
    ...
    expect(data.written).toContain(path.join('src', 'zcl_demo', 'zcl_demo.clas.json'));
    expect(data.written).toContain(path.join('src', 'zcl_demo', 'zcl_demo.clas.abap'));
  });
  ...
  it('--include-tests also writes the testclasses part', async () => {
    ...
    expect(res.exitCode).toBeUndefined();
    expect(fs.existsSync(path.join(cwd, 'src', 'zcl_demo', 'zcl_demo.clas.testclasses.abap'))).toBe(true);
  });
  ...
  it('existing identical files are skipped; differing files need --overwrite', async () => {
    ...
    expect(data.skipped).toContain(path.join('src', 'zcl_demo', 'zcl_demo.clas.abap'));
    expect(data.written).toContain(path.join('src', 'zcl_demo', 'zcl_demo.clas.json'));
