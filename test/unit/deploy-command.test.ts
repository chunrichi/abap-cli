/**
 * abap extension deploy command — commander-level behavior for `--package` default and
...
function program() {
  const p = makeProgram();
  registerExtensionCommand(p);
  return p;
}

describe('abap extension deploy command — --package default and --tr rules', () => {
  beforeEach(() => {
    deployBundled.mockClear();
  });

  it('defaults --package to $TMP and runs without --tr', async () => {
    const res = await runCommand(program(), ['extension', 'deploy', '--dry-run', '--json']);
  ...
  it('omitting --tr with --package $TMP is accepted', async () => {
    const res = await runCommand(program(), ['extension', 'deploy', '--dry-run', '--package', '$TMP', '--json']);
    expect(deployBundled).toHaveBeenCalledTimes(1);
    expect(stdoutHasError(res.stdout)).toBe(false);
  });

  it('rejects --package ZFOO without --tr with NO_TRANSPORT', async () => {
    const res = await runCommand(program(), ['extension', 'deploy', '--dry-run', '--package', 'ZFOO', '--json']);
  ...
  it('accepts --package ZFOO with --tr', async () => {
    const res = await runCommand(program(), ['extension', 'deploy', '--dry-run', '--package', 'ZFOO', '--tr', 'NDK123456', '--json']);
    expect(deployBundled).toHaveBeenCalledTimes(1);
    const [, opts] = deployBundled.mock.calls[0]!;
